...
Nodes run sequentially and may send messages to other nodes through rule derivations.
...
\section{Action Facts}

We use action facts to change the behavior of the runtime system. In particular, one can change the
order in which nodes are evaluated by adding priorities. Node priority works at the processor level
so that each processor can make a local decision about which node of the graph to run next. Without
coordination, nodes are picked arbitrarily (a FIFO approach is used by default).

The use of linear logic makes this possible since the order in which nodes are executed
...
\begin{description}
   ...
   \item[\texttt{type linear action schedule-next(node)}]: The processor will fetch the highest priority node's priority $P$ and set the action's argument node's priority as $P + 1.0$. If using the priorities
...
To understand how coordination directives work, we will present some programs that use them.
...
In this chapter we presented the current set of coordination directives, including sensing and action facts. The use of such facilities allows the programmer to write derivation rules that change how
the runtime system schedules computation thus improving the executing time and possibly the final program
results. As future work, we intend to extend the set of available directives and write one or two extra
